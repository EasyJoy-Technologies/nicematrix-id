...
export const defaultManagementApi = Object.freeze({
  resource: {
     ...
     * NiceMatrix: uses id.nicematrix.com as base instead of logto.app
     */
    indicator: `https://id.nicematrix.com/${defaultTenantId}/api`,
...
/**
 * Generate the Management API resource indicator for a given tenant.
 * NiceMatrix: uses id.nicematrix.com/<tenantId>/<path> instead of <tenantId>.logto.app/<path>
 */
export function getManagementApiResourceIndicator<TenantId extends string>(
  tenantId: TenantId
): `https://id.nicematrix.com/${TenantId}/api`;
export function getManagementApiResourceIndicator<TenantId extends string, Path extends string>(
  tenantId: TenantId,
  path: Path
): `https://id.nicematrix.com/${TenantId}/${Path}`;

export function getManagementApiResourceIndicator(tenantId: string, path = 'api') {
  return `https://id.nicematrix.com/${tenantId}/${path}`;
}
