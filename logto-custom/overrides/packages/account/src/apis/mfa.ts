/*
 * [NiceMatrix override] vs upstream packages/account/src/apis/mfa.ts (v1.43.0). Verbatim copy
 * EXCEPT the `updateMfaSettings` payload type at the bottom of the file.
 *
 * `PATCH /api/my-account/mfa-settings` now takes `isEnabled` as the authoritative two-step
 * verification switch (it writes both `mfa.enabled` and `mfa.skipMfaOnSignIn`, which can
 * therefore never contradict each other). `skipMfaOnSignIn` is still accepted by the server for
 * older clients, so the type allows either. See `docs/mfa-explicit-optin-plan.md` §4.
 *
 * On upstream sync: re-copy this file and re-apply the payload type.
 */
import {
...
export const updateMfaSettings = async (
  accessToken: string,
  verificationRecordId: string,
  // [NiceMatrix override] `isEnabled` is the switch the Account Center uses;
  // `skipMfaOnSignIn` remains valid for backwards compatibility.
  payload: { isEnabled: boolean } | { skipMfaOnSignIn: boolean }
