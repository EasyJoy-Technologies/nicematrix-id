/*
 * [NiceMatrix override] vs upstream packages/account/src/pages/Security/index.test.tsx
 * (v1.43.0). Verbatim copy EXCEPT the two-step verification scenarios, which now speak the
 * explicit-opt-in contract: `/mfa-settings` returns the resolved state (`isEnabled` /
 * `hasUsableFactor` / `usableFactors`) and the toggle writes `isEnabled`.
 * See `docs/mfa-explicit-optin-plan.md` §4/§6.
 *
 * On upstream sync: re-copy this file and re-apply the `mfaSettings()` helper plus the
 * scenarios marked with "[NiceMatrix override]".
 */
...
/**
 * [NiceMatrix override] Build a `/mfa-settings` response. Defaults mirror the common case:
 * two-step verification off, with nothing bound that could turn it on.
 */
const mfaSettings = ({
  isEnabled = false,
  hasUsableFactor = isEnabled,
  usableFactors = hasUsableFactor ? [MfaFactor.TOTP] : [],
}: {
  isEnabled?: boolean;
...
describe('<Security />', () => {
  beforeEach(() => {
    ...
    // [NiceMatrix override] default: off, and nothing bound to turn it on with.
    mockGetMfaSettings.mockResolvedValue(mfaSettings());
    mockGetMfaVerifications.mockResolvedValue([]);
    mockUpdateMfaSettings.mockResolvedValue(mfaSettings());
  ...
  it('renders configured MFA method rows from mocked API data', async () => {
    mockGetMfaSettings.mockResolvedValue(mfaSettings({ isEnabled: true }));
    mockGetMfaVerifications.mockResolvedValue(configuredMfaVerifications);
  ...
  it('navigates to verified action when enabling MFA without verification ID', async () => {
    // [NiceMatrix override] the switch is only operable once a factor is bound.
    mockGetMfaSettings.mockResolvedValue(mfaSettings({ hasUsableFactor: true }));
  ...
  it('updates MFA settings when enabling MFA with verification ID', async () => {
    // [NiceMatrix override] a bound factor is required before the switch can be turned on.
    mockGetMfaSettings.mockResolvedValue(mfaSettings({ hasUsableFactor: true }));
    mockUpdateMfaSettings.mockResolvedValue(mfaSettings({ isEnabled: true }));
    ...
    await waitFor(() => {
      // [NiceMatrix override] the switch writes the resolved state, not the raw opt-out flag.
  ...
  it('[NiceMatrix] disables the switch and warns when no factor is bound', async () => {
    mockGetMfaSettings.mockResolvedValue(mfaSettings({ hasUsableFactor: false }));
    ...
    await waitFor(() => {
      expect(
        queryByText('account_center.security.no_verification_method_warning')
      ).not.toBeNull();
    });

    const toggle = getByLabelText('Toggle switch') as HTMLInputElement;
  ...
  it('updates MFA settings and UI state after confirming disable', async () => {
    mockGetMfaSettings.mockResolvedValue(mfaSettings({ isEnabled: true }));
    mockUpdateMfaSettings.mockResolvedValue(
      mfaSettings({ isEnabled: false, hasUsableFactor: true })
    );
    ...
    await waitFor(() => {
      // [NiceMatrix override] see above - the switch writes the resolved state.
      expect(mockUpdateMfaSettings).toHaveBeenCalledWith('access-token', 'verification-record-id', {
        isEnabled: false,
      });
    });
