...
import { passkeySuccessRoute, backupCodesGenerateRoute } from '@ac/constants/routes';
...
const PasskeyBinding = () => {
  ...
  const handleSubmitName = useCallback(async () => {
    ...
    // NiceMatrix override: after a passkey is added, make sure the account has at
    // least one NON-device-bound recovery factor so the user cannot be locked out
    // if this device/passkey becomes unusable (lost device, browser wipe, RP ID
    // change). If BackupCode is enabled in SIE and the user has none, detour them
    // through the backup-codes generation flow before showing the success page.
    // EmailVerificationCode / PhoneVerificationCode are implicit fallbacks, but
    // only if the user actually has a primaryEmail / primaryPhone, so we don't
    // treat them as sufficient here — BackupCode is always offline-capable.
    const backupCodeEnabledInSie =
      experienceSettings?.mfa.factors.includes(MfaFactor.BackupCode) ?? false;

    if (backupCodeEnabledInSie) {
      const [verificationsError, verifications] = await getMfaVerificationsRequest();
      if (!verificationsError && verifications) {
        const hasBackupCode = verifications.some(({ type }) => type === MfaFactor.BackupCode);
        const hasTotp = verifications.some(({ type }) => type === MfaFactor.TOTP);
        const needsBackup = !hasBackupCode && !hasTotp;
        if (needsBackup) {
          navigate(backupCodesGenerateRoute, { replace: true });
          return;
        }
      }
    }
    ...
    addWebAuthnRequest,
    experienceSettings?.mfa.factors,
