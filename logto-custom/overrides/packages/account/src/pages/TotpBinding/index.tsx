  ...
  useEffect(() => {
    ...
    const generateSecret = async () => {
      ...
      if (result) {
        ...
        // [NiceMatrix override] vs upstream: the TOTP issuer (authenticator-app
        // account title) is the fixed brand name instead of
        // `window.location.hostname` (e.g. `id.nicematrix.com`). This frontend
        // bundle (packages/account) cannot import from packages/core, so the
        // literal mirrors `core/src/constants/mfa-issuer.ts` (mfaIssuerName).
        // Keep both in sync. Pure display change; not part of the TOTP HMAC.
        const issuer = 'NiceMatrix ID';
        const keyUri = `otpauth://totp/${encodeURIComponent(issuer)}:${encodeURIComponent(displayName)}?secret=${result.secret}&issuer=${encodeURIComponent(issuer)}`;
        try {
