/*
 * [NiceMatrix override] vs upstream
 *   logto-upstream/packages/core/src/libraries/session/consent.ts (1.40.1)
 * Verbatim copy with one marked `[NiceMatrix override]` delta block: after the
 * grant is saved, persist the device-session binding (device_ref + app_slug)
 * from the authorization request's extra params, keyed by the final grant id.
 *
 * `consent()` is the single grant-creation point for the web
 * (authorization_code) flow — it is called by BOTH the third-party manual
 * consent route (routes/interaction/consent) and the first-party auto-consent
 * middleware (middleware/koa-auto-consent), so this one delta covers all web
 * logins. The unified extraTokenClaims producer then emits the claims on the
 * access token and every refresh rotation (keyed by grant id).
 *
 * Fail-soft: missing/invalid params or absent table → silent no-op (token
 * carries no device claim → backend lease check fail-open exempts it).
 *
 * On upstream sync, re-diff and re-apply the marker block.
 */
...
// [NiceMatrix override] device-session binding store.
import { upsertGrantDevice } from '../../oidc/grant-device-store.js';
...
export const consent = async ({
  ...
  // [NiceMatrix override] Bind device_ref + app_slug (from the /authorize extra
  // params) to this grant id so the unified extraTokenClaims producer emits
  // them on the access token and every refresh rotation. Fail-soft: missing /
  // invalid params or absent table is a silent no-op.
  const { device_ref: deviceRef, app_slug: appSlug } = interactionDetails.params;
  await upsertGrantDevice(queries.pool, finalGrantId, deviceRef, appSlug);
