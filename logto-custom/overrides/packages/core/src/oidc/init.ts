...
export default function initOidc(
  ...
  const oidc = new Provider(envSet.oidc.issuer, {
    ...
    ttl: {
      ...
      // [NiceMatrix override 2026-06-10] Grant absolute ceiling raised
      // 180 -> 365 days (Xianglin). Effective login validity per session is
      // still min(refresh-token idle TTL = 60d default, this ceiling): a user
      ...
      // packages/core/src/oidc/init.ts (logto v1.40.1); keep it in sync on
