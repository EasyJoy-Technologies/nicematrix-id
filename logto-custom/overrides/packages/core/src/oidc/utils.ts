...
export const buildLoginPromptUrl = (
  ...
  // [NiceMatrix override] Pass-through App-handoff params so the experience SPA can
  // capture them into sessionStorage and decide whether to take over native social
  // sign-in. Without these 3 lines, oidc-provider would strip non-standard params on
  // the /oidc/auth -> /sign-in redirect. PC entries don't append these, so behaviour
  // is byte-identical to upstream when params are absent.
