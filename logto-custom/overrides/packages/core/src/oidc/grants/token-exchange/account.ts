 ...
 *   logto-upstream/packages/core/src/oidc/grants/token-exchange/account.ts (1.40.1)
 * Near-verbatim copy with marked `[NiceMatrix override]` deltas: `validateSubjectToken`
 * now also surfaces the subject token's `context` (jsonb) on the impersonation
 * branch, so the token-exchange grant can persist the device-session binding
 * (device_ref + app_slug) the backend put there when minting the subject token.
 *
 * Only the impersonation branch carries context (that is the native social
 * login path: backend POST /api/subject-tokens with context). JWT / opaque
 * access-token re-exchanges and personal-access-tokens have no context and
 * return undefined — correct, they are not device logins.
 *
 * On upstream sync, re-diff and re-apply the marker blocks.
 */
...
// [NiceMatrix override] Shape of the data we surface to the grant. `context`
// is an opaque JSON object the backend stored on the subject token.
type SubjectTokenValidationResult = {
  userId: string;
...
const validateImpersonationToken = async (
  ...
  subjectToken: string
): Promise<SubjectTokenValidationResult> => {
  ...
  return {
    ...
    // [NiceMatrix override] surface the stored context (device_ref / app_slug).
    // eslint-disable-next-line no-restricted-syntax -- context column is JsonObject at runtime
    context: token.context as Record<string, unknown> | undefined,
  };
};
...
export const validateSubjectToken = async ({
  ...
  AccessToken,
  jwtVerificationOptions,
}: ValidateSubjectTokenParams): Promise<SubjectTokenValidationResult> => {
  const {
