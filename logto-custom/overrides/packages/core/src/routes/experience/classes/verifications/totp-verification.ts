 ...
 *   logto-upstream/packages/core/src/routes/experience/classes/verifications/totp-verification.ts
 *
 * Delta (search "[NiceMatrix]"): the TOTP `otpauth://` issuer — what the
 * authenticator app shows as the account title — is now the fixed brand name
 * `mfaIssuerName` ('NiceMatrix ID') instead of the request hostname
 * (`ctx.URL.hostname`, e.g. `id.nicematrix.com`). Pure display change; the
...
export class TotpVerification implements MfaVerificationRecord<VerificationType.TOTP> {
  ...
  async generateNewSecret(
    _ctx: WithLogContext
  ): Promise<{ secret: string; secretQrCode: string }> {
    this.#secret = generateTotpSecret();

    // [NiceMatrix] issuer is a fixed brand name now; the request context is no
    // longer needed to derive it (kept in the signature for caller stability).
    const secretQrCode = await this.generateSecretQrCode();
  ...
  /**
   * The QR code is generated using the secret, the fixed brand issuer name, and
   * user information. The QR code can be used to bind the TOTP secret to the
   * user's authenticator app. The QR code is returned as a base64 encoded image.
   */
  private async generateSecretQrCode() {
