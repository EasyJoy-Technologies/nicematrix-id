 ...
 *   logto-upstream/packages/core/src/routes/interaction/additional.ts
 *
 * Delta (search "[NiceMatrix]"): the TOTP `otpauth://` issuer (authenticator-app
 * account title) is the fixed brand name `mfaIssuerName` ('NiceMatrix ID')
 * instead of the request hostname. Only the two TOTP `keyuri()` calls change;
 * the WebAuthn `rpId: ctx.URL.hostname` usages are intentionally untouched (the
 * relying-party ID must remain the real domain). Pure display change.
 */
import {
  InteractionEvent,
  MfaFactor,
  ...
  router.post(
    ...
    async (ctx, next) => {
      ...
      if (isRegisterInteractionResult(profileVerifiedInteraction)) {
        ...
        const keyUri = authenticator.keyuri(user ?? 'Unnamed User', mfaIssuerName, secret);
