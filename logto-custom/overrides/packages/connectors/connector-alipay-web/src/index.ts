...
const getUserInfo =
  ...
  async (data) => {
    ...
    // NiceMatrix patch: identityId priority union_id > user_id > open_id.
    // - union_id: same user across all apps in the same Alipay Application Grouping (preferred).
    // - user_id (2088xxx): legacy apps in traditional (non-OpenID) mode.
    // - open_id: post-2024-04-01 OpenID mode default; unique only within one app.
    const {
      user_id: userId,
      open_id: openId,
      union_id: unionId,
    ...
    const id = unionId ?? userId ?? openId;
